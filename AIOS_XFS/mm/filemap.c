...
#ifdef CONFIG_AIOS
int __AIOS_filemap_fdatawrite_range(struct address_space *mapping, loff_t start,
				loff_t end, int sync_mode)
{
	int ret;
	struct writeback_control wbc = {
	...
	wbc_attach_fdatawrite_inode(&wbc, mapping->host);
	ret = AIOS_do_writepages(mapping, &wbc);
	wbc_detach_inode(&wbc);
	return ret;
}
#endif
...
#ifdef CONFIG_AIOS
int AIOS_file_write_range(struct file *file, loff_t lstart, loff_t lend)
{
	int err = 0, err2;
	struct address_space *mapping = file->f_mapping;

	if (mapping_needs_writeback(mapping)) {
		err = __AIOS_filemap_fdatawrite_range(mapping, lstart, lend,
						 WB_SYNC_ALL);
		/* See comment of filemap_write_and_wait() */
		if (err == -EIO)
			printk(KERN_ERR "[AIOS ERROR] %s:%d\n", __func__, __LINE__);
...
EXPORT_SYMBOL(AIOS_file_write_range);
#endif

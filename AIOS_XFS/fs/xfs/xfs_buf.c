...
struct xfs_log_vec	*chead_lv=NULL;
EXPORT_SYMBOL(chead_lv);

struct xfs_log_vec	*ctail_lv=NULL;
EXPORT_SYMBOL(ctail_lv);

struct xfs_cil_ctx	*chead_ctx=NULL;
EXPORT_SYMBOL(chead_ctx);

struct xfs_cil_ctx	*ctail_ctx=NULL;
EXPORT_SYMBOL(ctail_ctx);

spinlock_t	AIOS_lv_lock;
EXPORT_SYMBOL(AIOS_lv_lock);

spinlock_t	AIOS_ctx_lock;
EXPORT_SYMBOL(AIOS_ctx_lock);

spinlock_t	AIOS_trans_lock;
EXPORT_SYMBOL(AIOS_trans_lock);
#endif
	...
	struct xfs_buf		*bp)
{
	...
	if (atomic_read(&bp->b_pin_count) && (bp->b_flags & XBF_STALE))
		xfs_AIOS_log_force(bp->b_target->bt_mount, 0);
	down(&bp->b_sema);
	...
	struct xfs_buf	*bp)
{
	...
	if(chead_ctx != NULL){
		for (ctx = chead_ctx; ctx; ) {
			struct xfs_cil_ctx *next = ctx->ctx_next;
			kmem_free(ctx);
			ctx = next;
		}
		chead_ctx = NULL;

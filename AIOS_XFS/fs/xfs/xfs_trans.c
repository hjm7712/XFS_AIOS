	...
	bool			regrant)
{
	...
	if (!regrant && (tp->t_flags & XFS_TRANS_PERM_LOG_RES)) {
		//printk("before xfs_defer_finish_noroll\n");
	...
	//printk("before xfs_log_commit_cil\n");
	xfs_log_commit_cil(mp, tp, &commit_lsn, regrant);
	//printk("after xfs_log_commit_cil\n");
	...
	bool			regrant)
{
	...
	xfs_AIOS_log_commit_cil(mp, tp, &commit_lsn, regrant);
...
out_unreserve:
	...
	xfs_trans_unreserve_and_mod_dquots(tp);
	if (tp->t_ticket) {
		commit_lsn = xfs_AIOS_log_done(mp, tp->t_ticket, NULL, regrant);
		if (commit_lsn == -1 && !error)
			error = -EIO;
		tp->t_ticket = NULL;
	}
...
#ifdef CONFIG_AIOS
int
xfs_AIOS_trans_commit(
	struct xfs_trans	*tp)
{
	return __xfs_AIOS_trans_commit(tp, false);
}
#endif
	...
	struct xfs_trans	*tp)
{
	...
	if (tp->t_ticket) {
		xfs_AIOS_log_done(mp, tp->t_ticket, NULL, false);
		tp->t_ticket = NULL;
	}
